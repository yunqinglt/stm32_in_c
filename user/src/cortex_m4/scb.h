...
typedef struct {
    ...
    __IO uint32_t SHCSR[1];
    __IO uint32_t CFSR[1];
    __IO uint32_t HFSR[1];
    __IO uint32_t MMFAR[1];
    __IO uint32_t BFAR[1];
    __IO uint32_t AFSR[1];

} scb_t;

#define scb     ((scb_t *) _SC)

#define cpuid   scb->CPUID[0]
#define icsr    scb->ICSR[0]

#define vtor    scb->VTOR[0]
#define aircr   scb->AIRCR[0]

#define scr     scb->SCR[0]
#define ccr     scb->CCR[0]

#define shpr(n)   scb->SHPR[n]

#define shcsr   scb->SHCSR[0]
#define cfsr    scb->CFSR[0]
#define mmfar   scb->MMFAR[0]
#define bfar    scb->BFAR[0]
#define afsr    scb->AFSR[0]
